...
export async function listEvents(tokens: Credentials): Promise<calendar_v3.Schema$Event[]> {
  if (!tokens) {
    console.error('No tokens provided for listEvents');
    return [];
  }
  try {
...
export async function createEvent(tokens: Credentials, event: calendar_v3.Schema$Event): Promise<calendar_v3.Schema$Event | null> {
    if (!tokens) {
        console.error('No tokens provided for createEvent');
        return null;
    }
    try {
        const auth = createOAuth2Client(tokens);
...
export async function deleteEvent(tokens: Credentials, eventId: string): Promise<boolean> {
    if (!tokens) {
        console.error('No tokens provided for deleteEvent');
        return false;
    }
    try {
        const auth = createOAuth2Client(tokens);
...
export async function rescheduleEvent(tokens: Credentials, eventId: string, start: string, end: string): Promise<calendar_v3.Schema$Event | null> {
    if (!tokens) {
        console.error('No tokens provided for rescheduleEvent');
        return null;
    }
    try {
        const auth = createOAuth2Client(tokens);
...
export async function findFreeTimeSlots(tokens: Credentials, { timeMin, timeMax, durationMinutes, userTimezone }: { timeMin: string, timeMax: string, durationMinutes: number, userTimezone: string }): Promise<{start: string, end: string}[]> {
    if (!tokens) {
        console.error('No tokens provided for findFreeTimeSlots');
        return [];
    }
    try {
        const auth = createOAuth2Client(tokens);
